...
package cn.iichen.quickshot.widget.loading;
...
public class DrawableTextView extends AppCompatTextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        this.enableCenterDrawables = array.getBoolean(R.styleable.DrawableTextView_enableCenterDrawables, false);
        this.enableTextInCenter = array.getBoolean(R.styleable.DrawableTextView_enableTextInCenter, false);
        this.radius = array.getDimensionPixelSize(R.styleable.DrawableTextView_radius, 0);
        if (VERSION.SDK_INT >= 21 && this.radius > 0) {
            this.setClipToOutline(true);
            this.setOutlineProvider(new DrawableTextView.RadiusViewOutlineProvider());
        }

        Rect bottomBounds;
        if (drawables[0] != null) {
            bottomBounds = drawables[0].getBounds();
            bottomBounds.right = array.getDimensionPixelSize(R.styleable.DrawableTextView_drawableStartWidth, drawables[0].getIntrinsicWidth());
            bottomBounds.bottom = array.getDimensionPixelSize(R.styleable.DrawableTextView_drawableStartHeight, drawables[0].getIntrinsicHeight());
        }

        if (drawables[1] != null) {
            bottomBounds = drawables[1].getBounds();
            bottomBounds.right = array.getDimensionPixelSize(R.styleable.DrawableTextView_drawableTopWidth, drawables[1].getIntrinsicWidth());
            bottomBounds.bottom = array.getDimensionPixelSize(R.styleable.DrawableTextView_drawableTopHeight, drawables[1].getIntrinsicHeight());
        }

        if (drawables[2] != null) {
            bottomBounds = drawables[2].getBounds();
            bottomBounds.right = array.getDimensionPixelSize(R.styleable.DrawableTextView_drawableEndWidth, drawables[2].getIntrinsicWidth());
            bottomBounds.bottom = array.getDimensionPixelSize(R.styleable.DrawableTextView_drawableEndHeight, drawables[2].getIntrinsicHeight());
        }

        if (drawables[3] != null) {
            bottomBounds = drawables[3].getBounds();
            bottomBounds.right = array.getDimensionPixelSize(R.styleable.DrawableTextView_drawableBottomWidth, drawables[3].getIntrinsicWidth());
            bottomBounds.bottom = array.getDimensionPixelSize(R.styleable.DrawableTextView_drawableBottomHeight, drawables[3].getIntrinsicHeight());
        }
        ...
        if (this.enableCenterDrawables && this.isCenterHorizontal | this.isCenterVertical) {
            ...
            if (this.mDrawables[0] != null && this.isCenterHorizontal) {
                ...
                if (this.enableTextInCenter && textNoEmpty) {
                    transX -= this.mDrawablesBounds[0].width() + this.getCompoundDrawablePadding() >> 1;
                }
            }
    ...
    public DrawableTextView setRadius(@Px int px) {
        this.radius = px;
        if (!(this.getOutlineProvider() instanceof DrawableTextView.RadiusViewOutlineProvider)) {
            this.setOutlineProvider(new DrawableTextView.RadiusViewOutlineProvider());
            this.setClipToOutline(true);
        } else {
